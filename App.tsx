      ...
      {gameState === 'PLAYING' && (
        <>
          <div className="absolute top-10 flex gap-8 text-white font-black text-2xl z-10 bg-red-600/20 px-6 py-3 rounded-full border border-red-500/50 backdrop-blur-xl">
            ...
            <div className={timeLeft < 5 ? "text-red-500 animate-pulse" : ""}>⏱️ {timeLeft}s</div>
          </div>
          {angpaos.map(angpao => (
  ...
  return (
    <div className="relative select-none transition-transform hover:scale-105 duration-500 shrink-0 z-10 mx-auto" style={{ width: containerSize, height: containerSize }}>
       ...
       <div className={`absolute inset-0 m-auto w-20 h-20 md:w-28 md:h-28 rounded-full border-[4px] md:border-[6px] flex flex-col items-center justify-center shadow-2xl z-10 bg-white transition-all ${isSoldOut ? 'shadow-[0_0_40px_rgba(212,175,55,0.7)]' : ''}`} style={{ borderColor: isSoldOut ? '#d4af37' : tierColor }}>
          ...
          {/* UPDATED: Table 14 displayed as 13A */}
...
export const App: React.FC = () => {
  ...
  return (
    <div className="h-full w-full flex flex-col font-sans cny-pattern no-swipe overflow-hidden bg-[#0d0101]">
      ...
      <main className="flex-1 w-full overflow-y-auto custom-scrollbar no-swipe pt-20 md:pt-24">
             ...
             <div className="flex flex-col items-center">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 w-full px-4">
                {Array.from({ length: 6 }).map((_, i) => (
                  <RoundTable key={i+1} tableId={i+1} seats={seats.filter(s => s.tableId === i+1)} tier={SeatTier.GOLD} config={config} mySelectedIds={mySelectedIds} onSeatClick={handleSeatClick} />
                ))}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 w-full px-4 mt-12">
                {Array.from({ length: 4 }).map((_, i) => (
                  <RoundTable key={i+7} tableId={i+7} seats={seats.filter(s => s.tableId === i+7)} tier={SeatTier.GOLD} config={config} mySelectedIds={mySelectedIds} onSeatClick={handleSeatClick} />
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 w-full px-4">
                {Array.from({ length: 4 }).map((_, i) => (
                  <RoundTable key={i+11} tableId={i+11} seats={seats.filter(s => s.tableId === i+11)} tier={SeatTier.SILVER} config={config} mySelectedIds={mySelectedIds} onSeatClick={handleSeatClick} />
                ))}
              </div>
            </div>
          </div>
        ...
        {view === 'cart' && (
          <div className="max-w-2xl mx-auto p-4 md:p-8 pb-48">
            <button onClick={() => setView('hall')} className="mb-6 flex items-center gap-2 text-[#d4af37] font-black uppercase text-[10px]"><ChevronLeft /> Back to Hall</button>
            <div className="bg-white rounded-[32px] shadow-2xl border-[6px] border-[#8b0000] overflow-hidden">
              <div className="bg-[#8b0000] p-6 text-[#fef9c3] flex justify-between items-center">
                  <h2 className="text-xl font-serif font-black uppercase tracking-widest">Cart</h2>
                  <Ticket className="w-6 h-6 text-[#d4af37]" />
              </div>
              <div className="p-6 space-y-4">
                  {mySelectedSeats.length === 0 ? <p className="text-center py-20 text-stone-300">No seats selected</p> : mySelectedSeats.map(s => (
                    <div key={s.id} className="flex items-center justify-between p-4 bg-stone-50 rounded-2xl border">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-stone-900 text-white rounded-xl flex flex-col items-center justify-center font-black">
                          {/* UPDATED: Cart display also shows 13A */}
                          <span className="text-[6px] opacity-40 uppercase">T-{s.tableId === 4 ? '3A' : s.tableId === 14 ? '13A' : s.tableId}</span>
                          <span className="text-sm">{s.seatNumber}</span>
                        </div>
                        <p className="font-black text-stone-800 uppercase text-xs">{config.tiers[s.tier].label} - RM{s.price.toFixed(2)}</p>
                      </div>
                      <button onClick={() => handleSeatClick(s.id)} className="text-red-500 hover:scale-110"><Trash2 className="w-5 h-5" /></button>
                    </div>
                  ))}
                  <div className="p-4 bg-stone-50 rounded-xl flex justify-end font-serif font-black text-xl text-stone-900">Total: RM {mySelectedSeats.reduce((acc, s) => acc + s.price, 0).toFixed(2)}</div>
                  <button onClick={() => setConfirmOpen(true)} disabled={mySelectedSeats.length === 0} className="w-full py-6 bg-[#d4af37] text-[#5c1a1a] rounded-3xl font-black text-xl uppercase shadow-xl hover:scale-[1.02] transition-all disabled:bg-stone-200">Checkout</button>
              </div>
            </div>
          </div>
        )}
      ...
      {view === 'hall' && (
        <div className="fixed bottom-6 left-6 z-[9999] flex flex-col gap-3">
          <button onClick={() => setShowGameModal(true)} className="px-6 py-4 bg-orange-600 text-white rounded-3xl font-black uppercase text-xs flex items-center gap-3 shadow-2xl border-2 border-white/20 animate-pulse"><Gamepad2 className="w-4 h-4" /> Catch Angpao</button>
          <button onClick={() => setView('cart')} className="px-8 py-4 bg-red-800 text-white rounded-3xl font-black uppercase text-xs flex items-center gap-3 shadow-2xl border-2 border-white/20"><ShoppingBag className="w-5 h-5" /> Book ({mySelectedIds.length})</button>
        </div>
      )}
      ...
      <PaymentModal isOpen={isPaymentOpen} timeLeft={timeLeft} onClose={handleCheckoutCleanup} amount={mySelectedSeats.reduce((acc, s) => acc + (pendingDetails[s.id]?.isMember ? s.price - 1 : s.price), 0)} count={mySelectedSeats.length} paymentConfig={config.payment} seats={seats} pendingDetails={pendingDetails} onConfirm={() => setView('hall')} />
