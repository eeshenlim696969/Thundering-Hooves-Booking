...
export const App: React.FC = () => {
  ...
  return (
    <div className="h-full w-full flex flex-col font-sans majestic-bg no-swipe overflow-hidden">
      {/* Background Animation CSS */}
      <style>{`
        @keyframes majestic-flow {
          0% { background-position: 0% 50%; }
          50% { background-position: 100% 50%; }
          100% { background-position: 0% 50%; }
        }
        .majestic-bg {
          background: linear-gradient(-45deg, #1a0202, #2d0a0a, #3f0e0e, #1a0202);
          background-size: 400% 400%;
          animation: majestic-flow 15s ease infinite;
          position: relative;
        }
        .majestic-bg::before {
          content: "";
          position: absolute;
          inset: 0;
          background-image: radial-gradient(circle at 50% 0%, rgba(212, 175, 55, 0.1), transparent 70%);
          pointer-events: none;
        }
      `}</style>
