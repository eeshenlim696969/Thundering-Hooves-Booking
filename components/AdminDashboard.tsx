...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const handleExportCSV = () => {
    ...
    const csvRows = filteredSeats.map(s => {
      ...
      const cat = s.paymentInfo?.category;
      
      const idInfo = cat === 'OUTSIDER' ? s.paymentInfo?.icNumber : s.paymentInfo?.studentId;
  ...
  const handleDelete = async (seat: SeatData) => {
    ...
    if (window.confirm(`Delete booking T${seat.tableId}-${seat.seatNumber}? Cannot be undone.`)) {
      setProcessingId(seat.id);
      try { 
        setRemovedIds(prev => new Set([...prev, seat.id]));
        await onReset(seat.id); 
      } catch (err) {
        setRemovedIds(prev => { const next = new Set(prev); next.delete(seat.id); return next; });
        alert("Failed.");
  ...
  const handleDecline = async (seat: SeatData) => {
    if (processingId) return;
    if (window.confirm('Decline booking?')) {
      setProcessingId(seat.id);
      try {
        setRemovedIds(prev => new Set([...prev, seat.id]));
        await onReset(seat.id); 
  ...
  const handleApprove = async (seat: SeatData) => {
    ...
    setProcessingId(seat.id);
    try { await onApprove(seat); } catch (err) { alert("Failed."); } finally { setProcessingId(null); }
  };
    ...
    <div className="flex flex-col lg:flex-row gap-6 md:gap-8 w-full relative">
      <aside className="lg:w-64 shrink-0">
        <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-[32px] p-4 md:p-6 sticky top-24 space-y-4">
          <nav className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-visible no-scrollbar">
            {[ { id: 'financials', label: 'Finance', icon: PieChart }, { id: 'map', label: 'Hall Map', icon: MapIcon }, { id: 'manifest', label: 'Guest List', icon: List } ].map(item => (
              <button key={item.id} onClick={() => setActiveView(item.id as any)} className={`flex items-center gap-3 px-5 py-4 rounded-2xl transition-all whitespace-nowrap min-w-fit flex-1 lg:flex-none ${activeView === item.id ? 'bg-[#d4af37] text-[#5c1a1a]' : 'text-white/40 hover:bg-white/5'}`}>
          ...
          <div className="pt-6 border-t border-white/5 space-y-3">
             <div className="flex items-center gap-2 px-2 mb-2">
                <Wand2 className="w-3 h-3 text-[#d4af37]" /><span className="text-[9px] font-black text-white/30 uppercase tracking-[0.2em]">🔮 FX Preview</span>
             </div>
             <div className="grid grid-cols-1 gap-2">
                <button onClick={() => onPreviewAura?.('PLATINUM')} className="w-full py-2 bg-red-600/10 hover:bg-red-600/20 text-red-400 rounded-xl text-[9px] font-black uppercase border border-red-600/20">Platinum</button>
                <button onClick={() => onPreviewAura?.('GOLD')} className="w-full py-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 rounded-xl text-[9px] font-black uppercase border border-amber-500/20">Gold</button>
                <button onClick={() => onPreviewAura?.('SILVER')} className="w-full py-2 bg-stone-500/10 hover:bg-stone-500/20 text-stone-400 rounded-xl text-[9px] font-black uppercase border border-stone-500/20">Silver</button>
             </div>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 min-w-0 view-transition">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={onLogout} className="px-6 py-2 bg-red-600 hover:bg-red-700 text-white font-black uppercase text-[10px] tracking-widest rounded-xl transition-all flex items-center gap-2 shadow-lg shadow-red-900/20"><LogOut className="w-4 h-4" /> Logout</button>
        </div>
            ...
            {onUpdatePrices && (
              <div className="bg-[#d4af37]/5 border border-[#d4af37]/20 p-6 rounded-[32px] mb-6">
                 <div className="flex justify-between items-center mb-4">
                    <h3 className="text-[#d4af37] font-black uppercase tracking-widest text-xs flex items-center gap-2"><DollarSign className="w-4 h-4" /> Global Price Override</h3>
                    {priceSaved && <span className="text-emerald-400 text-xs font-black uppercase tracking-widest animate-pulse">Updated!</span>}
                 </div>
                 ...
                 <div className="mt-4 flex justify-end">
                    <button onClick={handleSavePrices} className="flex items-center gap-2 px-6 py-3 bg-[#d4af37] text-stone-900 font-black uppercase text-[10px] tracking-widest rounded-xl hover:bg-[#fff176] transition-colors"><Save className="w-4 h-4" /> Update Prices</button>
                 </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="bg-white/5 border border-white/10 p-8 rounded-[32px] flex flex-col gap-4">
                ...
                <div className="pt-4 border-t border-white/5">
                  <p className="text-[10px] text-[#d4af37] uppercase tracking-widest mb-2 font-black">Net Profit</p>
        ...
        {activeView === 'manifest' && (
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
              ...
              <button onClick={handleExportCSV} className="w-full md:w-auto flex items-center justify-center gap-2 px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg transition-all"><FileSpreadsheet className="w-4 h-4" /><span>Export CSV</span></button>
            </div>
              ...
              <table className="w-full text-left min-w-[1100px]">
                ...
                <tbody className="divide-y divide-white/5">
                  {filteredSeats.map(s => {
                    const { table, seat } = parseId(s.id);
                    const cat = s.paymentInfo?.category;
                    const idText = cat === 'OUTSIDER' ? s.paymentInfo?.icNumber : s.paymentInfo?.studentId;
                    ...
                    return (
                      <tr key={s.id} className="text-white/80 group hover:bg-white/[0.02]">
                        ...
                        <td className="px-6 py-4">
                           ...
                           {cat === 'OUTSIDER' && <span className="flex items-center gap-2 text-[9px] font-black uppercase text-amber-400"><Car className="w-3 h-3" /> Outsider</span>}
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end items-center gap-2">
                             {s.status === SeatStatus.PENDING && (
                               <>
                                 <button onClick={() => handleApprove(s)} className="p-2 text-emerald-500 hover:bg-emerald-500/10 rounded-lg transition-colors" title="Approve"><CheckCircle2 className="w-5 h-5" /></button>
                                 <button onClick={() => handleDecline(s)} className="p-2 text-red-500 hover:bg-red-500/10 rounded-lg transition-colors" title="Decline"><XCircle className="w-5 h-5" /></button>
                               </>
                             )}
